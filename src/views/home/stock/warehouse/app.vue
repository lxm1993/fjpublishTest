...
<div class="warehouse">
	<!--模态框-->
	<el-dialog title="往来单位[供应商]选择框" :visible.sync="dialogOneVisible1">
	  <el-form label-width="100px" :label-position="labelPosition" :model="wlForm" class="demo-form-inline">
		<el-input placeholder="请输入内容" v-model="select">
            <el-select v-model="input" slot="prepend" placeholder="请选择" style="width: 100px;">
                <el-option label="快速查询" value="0"></el-option>
                <el-option label="按编号查询" value="1"></el-option>
                <el-option label="按简名查询" value="2"></el-option>
...
<el-dialog  title="往来单位-信息框" :visible.sync="dialogOneVisible2" >
    <div class="app_form">
    	<el-form :model="formInline" label-width="100px" :rules="wldw" ref="formInline">
    		<el-row>
    			<el-col>
		  ...
		  <el-row>
		  	...
		  	<el-col :span="8">
		  		<el-form-item label="价格等级">
				    <el-select size="small" v-model="formInline.unit_grade" placeholder="请选择">
				      <el-option label="预设售价1" value="1"></el-option>
				      <el-option label="预设售价2" value="2"></el-option>
				      <el-option label="预设售价3" value="3"></el-option>
				    </el-select>
				</el-form-item>
		  	</el-col>
		  </el-row>
...
<el-dialog title="地址" :visible.sync="dialogOneVisible3">
  ...
  <el-table :data="addressTableData" border height="230" style="width: 100%">
    ...
    <el-table-column  prop="city" label="省市区" width="180"></el-table-column>
    ...
    <el-table-column label="操作" width="150">
	      <template scope="scope">
	        ...
	        <el-button @click="dialogOneShow4"  type="text" size="small" value="选中" title="修改本行">修改</el-button>
...
<!---------新增详细地址的模态框-------------->
<el-dialog size="tiny" title="新增地址" :visible.sync="dialogOneVisible4">
    <div class="app_form">
    	<el-form :model="dizhiForm" label-width="100px" :rules="xzdz" ref="dizhiForm">
		   ...
		   <el-row>
		   	  ...
		   	  <el-col :span="12">
		   	  	 <el-form-item label="邮政编码" prop="dzcode"><el-input size="small" v-model="dizhiForm.dzcode"></el-input></el-form-item>
		   	  </el-col>
		   </el-row>
		   <el-row>
		   	<el-col>
...
<el-dialog size="tiny" title="仓库选择框" :visible.sync="dialogThreeVisible1">
	...
	<!--主要数据表格-->
  <el-table :data="cktableData" border height="230" style="width: 100%" @selection-change="handleSelectionChange">
    ...
    <el-table-column label="操作" width="150">
      <template scope="scope">
      	<el-button @click.native.prevent="ckchoiceRow(scope.$index, cktableData)" type="text" size="small">选中</el-button>
        <el-button  @click.native.prevent="deleteRow(scope.$index, cktableData)" type="danger" size="small" value="删除" title="删除本行">删除</el-button>
      </template>
    </el-table-column>
  </el-table>
 </el-form>
...
<el-dialog size="tiny" class="dialogfive1" title="录单配置" :visible.sync="dialogFiveVisible1">
    ...
    <el-table-column prop="name" label="配置类名"></el-table-column>
    <el-table-column label="操作" width="150">
      <template scope="scope">
...
<el-dialog title="商品套餐选择框" :visible.sync="dialogSixVisible1">
    <div >
		<el-input placeholder="请输入内容" v-model="select">
            <el-select v-model="input1" slot="prepend" placeholder="请选择" style="width: 100px;">
	...
	<!--主要数据表格-->
  <el-table :data="tcTableData" border height="230" style="width: 100%">
	<el-table-column  prop="tcbm" label="套餐编码" ></el-table-column>
    <el-table-column prop="tcmc" label="套餐名称"></el-table-column>
    <el-table-column label="操作" width="150">
      <template scope="scope">
      	<el-button @click.native.prevent="sptcchoice(scope.$index, tcTableData)" type="text" size="small" value="选中" title="选中本行">选中</el-button>
        <el-button  @click.native.prevent="deleteRow(scope.$index, tcTableData)" type="danger" size="small" value="删除" title="删除本行">删除</el-button>
      </template>
    </el-table-column>
  </el-table>
  <!--显示记录-->
...
<!------------新增套餐-------------->
<el-dialog title="新增商品套餐" :visible.sync="dialogSixVisible2">
    <div class="diaTop" >
    	<el-form :model="xztcForm" class="demo-form-inline" label-width="100px" :rules="xztc" ref="xztcForm">
    		<el-row>
    			<el-col :span="6">
    				<el-form-item label="套餐名称" prop="tcmc">
					   <el-input size="small" v-model="xztcForm.tcmc" ></el-input>
					</el-form-item>
    			</el-col>
    			<el-col :span="6">
    				<el-form-item label="套餐编码" prop="tcbm">
					    <el-input size="small" v-model="xztcForm.tcbm" ></el-input>
					  </el-form-item>
    			</el-col>
    			<el-col :span="6">
    				<el-form-item label="套餐条码" prop="tctm">
		    <el-input size="small" v-model="xztcForm.tctm" ></el-input>
		  </el-form-item>
    			</el-col>
    			<el-col :span="6">
    				 <el-form-item label="套餐打印方式">
		    <el-select  size="small" v-model="xztcForm.tcdyfs" >
		 ...
		 <el-row>
		 	<el-col :span="24">
		 		<el-form-item class="sixRem" label="备注">
			    	<el-input size="small" v-model="xztcForm.bz" ></el-input>
			  	</el-form-item>
		 	</el-col>
		 </el-row>
	...
	<!--主要数据表格-->
  <el-table :data="xztcTableData" border height="230" style="width: 100%">
	<el-table-column  prop="tcspbm" label="套餐商品编号" width="150"></el-table-column>
    <el-table-column prop="tcspmc" label="套餐商品名称" width="200"></el-table-column>
    <el-table-column  prop="dw" label="单位" width="150"></el-table-column>
    <el-table-column prop="sl" label="数量" width="200"></el-table-column>
    <el-table-column  prop="dj" label="单价" width="150"></el-table-column>
    <el-table-column prop="je" label="金额" width="200"></el-table-column>
    <el-table-column  prop="zt" label="状态" width="150"></el-table-column>
    <el-table-column label="操作" width="150">
		      <template scope="scope">
		      	<el-button @click.native.prevent="xztcchoice(scope.$index, xztcTableData)" type="text" size="small" value="选中" title="选中本行">选中</el-button>
		        <el-button  @click.native.prevent="deleteRow(scope.$index, xztcTableData)" type="danger" size="small" value="删除" title="删除本行">删除</el-button>
		      </template>
    </el-table-column>
  </el-table>
  <!--显示记录-->
...
<el-dialog size="tiny" title="科目选择框" :visible.sync="dialogKemuVisible1">
	<el-form ref="kmForm" :model="kmForm" label-width="80px">
			<el-input placeholder="请输入内容" v-model="kmForm.select" >
                <el-select v-model="kmForm.input" slot="prepend" placeholder="请选择" style="width: 100px;">
                    <el-option label="快速查询" value="1"></el-option>
                    <el-option label="按编号查询" value="2"></el-option>
		...
		<!--主要数据表格-->
	  <el-table :data="kmtableData" border height="230" style="width: 100%">
		<el-table-column  prop="date" label="科目编号" width="150"></el-table-column>
	    <el-table-column prop="name" label="科目名称" width="200"></el-table-column>
	    <el-table-column label="操作" width="150">
	      <template scope="scope">
	      	<el-button @click.native.prevent="kmchoiceRow(scope.$index, kmtableData)" type="text" size="small">选中</el-button>
	        <el-button  @click.native.prevent="deleteRow(scope.$index, kmtableData)" type="danger" size="small" value="删除" title="删除本行">删除</el-button>
	      </template>
	    </el-table-column>
	  </el-table>
	 </el-form>
	  ...
	  <!--操作按钮-->
	  <div slot="footer" class="dialog-footer">
	  	<el-button size="small" @click="dialogKemuVisible1=false">关闭</el-button>
	  </div>
	</el-dialog>

<el-dialog v-drag title="库存商品选择框" :visible.sync="kcdialogVisible1">
		<el-form label-width="100px" :label-position="labelPosition" :model="wlForm" class="demo-form-inline">
			<el-input placeholder="请输入内容" v-model="wlForm.supply_select">
                <el-select v-model="wlForm.supply_input1" slot="prepend" placeholder="请选择" style="width: 100px;">
                    <el-option label="快速查询" value="1"></el-option>
...
<div class="demo-input-size">
 		<el-form label-width="80px" :model="formLabelAlign">
 			<el-row>
 				<el-col :span="6">进货入库单</el-col>
 				<el-col :span="3" :offset="9"> <el-form-item><el-checkbox-group v-model="form.type"><el-checkbox label="使用往来单位商品编号" name="type"></el-checkbox></el-checkbox-group>
  </el-form-item></el-col>
 				<el-col :span="3" ><el-form-item label="录单日期"><el-date-picker size="mini" v-model="value1" type="date" placeholder="选择日期"></el-date-picker></el-form-item></el-col>
...
<div class="demo-input-size new_screen">
		<el-form ref="form" :model="mainForm" label-width="80px">
			<el-row>
				<el-col :span="3"><el-form-item label="供货单位">
			        <el-input size="small" v-model="mainForm.ghdw" icon="more" :on-icon-click="dialogOneShow1"></el-input>
			    </el-form-item></el-col>
			    <el-col :span="3"><el-form-item label="收货仓库">
			        <el-input size="small" v-model="mainForm.shck" icon="more" :on-icon-click="dialogThreeShow1"></el-input>
			    </el-form-item></el-col>
			    <el-col :span="2"><el-form-item label="经手人">
			        <el-input size="small" v-model="mainForm.jsr" icon="more" :on-icon-click="dialogTwoShow1"></el-input>
			    </el-form-item></el-col>
			    <el-col :span="2">
			    	<el-form-item label="部门">
					    <el-select v-model="mainForm.bm" placeholder="自定义" size="small">
					      <el-option label="后勤部" value="1"></el-option>
					      <el-option label="维修部" value="2"></el-option>
					    </el-select>
					  </el-form-item>
			    </el-col>
			    <el-col :span="4"><el-form-item label="摘要">
			        <el-input size="small" v-model="mainForm.zy"></el-input>
			    </el-form-item></el-col>
			    <el-col :span="3"><el-form-item label="附加说明">
			        <el-input size="small" v-model="mainForm.fjsm" icon="more" :on-icon-click="dialogFourShow1"></el-input>
			    </el-form-item></el-col>
			    <el-col :span="2">
			    	<el-form-item label="票据类型">
					    <el-select v-model="mainForm.pjlx" placeholder="自定义" size="small">
					      <el-option label="收据" value="shanghai"></el-option>
					      <el-option label="税票" value="beijing"></el-option>
					    </el-select>
					  </el-form-item>
			    </el-col>
			</el-row>
	...
	<div class="list_table">
		<el-table :data="tableData" border height="532" style="width: 100%" @selection-change="handleSelectionChange">
		    ...
		    <el-table-column prop="order_name" label="商品名称" width="120">
		    	<template scope="scope">
		          <el-input v-model="select" icon="more" :on-icon-click="kcdialogShow1"></el-input>
		        </template>
		    </el-table-column>
...
<!--<el-form ref="form" :model="form" label-width="80px">-->
		<el-row>
		  ...
		  <el-col :span="1" align="right" :offset="8"><label>合计重量：</label></el-col>
		  <el-col :span="1" ><label>0</label></el-col>

          <el-col :span="1" align="right"><label>合计数量：</label></el-col>
		  <el-col :span="1" ><label>0</label></el-col>
		  ...
		  <el-col :span="1" align="right"><label>优惠金额：</label></el-col>
		  <el-col :span="1" ><label>0</label></el-col>

		  <el-col :span="1" align="right"><label>优惠后金额：</label></el-col>
		  <el-col :span="1" ><label>0</label></el-col>
		</el-row>


		  <el-form ref="fkForm" :model="fkForm" label-width="80px">
			<el-row>
				<el-col :span="3"><el-form-item label="付款账户">
			        <el-input size="small" v-model="fkForm.fkzh" icon="more" :on-icon-click="dialogKemuShow1"></el-input>
			    </el-form-item></el-col>
			    <el-col :span="3"><el-form-item label="付款金额">
			        <el-input size="small" v-model="fkForm.fkje"></el-input>
			    </el-form-item></el-col>

			   <el-col :span="1" align="right" :offset="12"><label>本次应付：</label></el-col>
			  <el-col :span="1" ><label>0</label></el-col>

			  <el-col :span="1" align="right"><label>此前应付：</label></el-col>
			  <el-col :span="1" ><label>0</label></el-col>

			  <el-col :span="1" align="right"><label>合计应付：</label></el-col>
			  <el-col :span="1" ><label>0</label></el-col>
			</el-row>
		</el-form>
		<div class="btn_all">
			<el-button size="small" @click="dialogFiveShow1">录单配置</el-button>
			<el-button size="small" >费用结算</el-button>
			<el-button size="small" @click="dialogSixShow1">选择套餐</el-button>
			<el-button size="small" @click="dialogSevenShow1">导入商品明细</el-button>
			<el-button size="small" >进货订单</el-button>
...
<script lang="babel">
      ...
      data:function() {
        return {
        	...
        	xztc:{
        		...
        		tctm:[
        		 {required: true, message: '套餐条码不能为空', trigger: 'blur'}
        		]
        	},
        	editForm: {
	            name: ''
  ...
  methods: {
    ...
    choiceRow(index, rows){
           this.mainForm.ghdw = rows[index].wl_name;
        },
    addresschoice(index, rows){
        this.formInline.unit_address = rows[index].city+rows[index].address;	
       },
    ckchoiceRow(index, rows){
           this.mainForm.shck = rows[index].ckname;
        },
    ywychoice(index, rows){
	    this.mainForm.jsr = rows[index].ywyname;	
	   },
	cychoice(index, rows){
	    this.mainForm.fjsm = rows[index].cycontent;	
	   },
	kmchoiceRow(index, rows){
	    this.fkForm.fkzh = rows[index].name;	
	   },
	sptcchoice(index, rows){
	   	this.dialogSixVisible1=false;
	   },
	xztcchoice(index, rows){
//	   this.mainForm.fjsm = rows[index].cycontent;	
	   },
